...
def extract_attendance():
    ...
    dte   = df["Date"]
    l = len(df)
    return names, rolls, times, l, dte
...
def add_attendance(name):
    ...
    if int(userid) in list(df['Roll']) and datetoday not in list(df["Date"]):
        with open(f'Attendance/Attendance.csv', 'a') as f:
            f.write(f'\n{username},{userid},{current_time},{datetoday}')
...
@app.route('/')
def home():
    names, rolls, times, l, dte = extract_attendance()
    return render_template('home.html', names=names, rolls=rolls, times=times, l=l, dte=dte, totalreg=totalreg(), datetoday2=datetoday2)
